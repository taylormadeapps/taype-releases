...
gain amount now push the shelf contour harder, and che musical turnover
"nose" now starts showing da around `+6 dB` and leaning in more clearly
...
Visualiser** window for the current track con the live response curve,
small `IN` / `OUT` spectrum toggles in the graph's top-right corner, a little
square `+` / `-` zoom column tucked in the top-left dB gutter beside the
scale labels, and split
`FILTER` / `EQ` control panels che mirror the strip's header power toggles
and EQ glyph buttons. The window title carries the track name, so the graph
itself stays uncluttered. Once open, che shared window follows whichever
track is currently selected. The popup now defaults to its tightest useful
graph view con `+18 dB` pinned at the top and `-18 dB` at the floor, and
...
in one centred
column so those knobs line up con the EQ rows, and the right-hand `EQ` panel
...
while showing signal in the EQ after the preamp and out of the EQ before
...
turning in graph paper.
In K-system views, che popup ladder keeps both the red `FS` ceiling tick and
...
che popup graph keeps `0 dBFS` pinned to the top edge, opens fully zoomed in
at `-36 dBFS`, and moves the floor in `6 dB` steps. In K-system modes it
still shows the K-scale labels and highlighted `0` reference line on che
same geometry.

Those popup `FILTER` and `EQ` headers also keep the same preset badge
language as the strip. Fai clic con il pulsante destro either one to load or save a filter or
