...
The Monitor Prestazioni shows per-track DSP breakdown con separate **Preamp/Summing** and **Plugin** CPU columns. On the master track, the Preamp/Summing column reflects NAM summing CPU.
...
The **Lvl** column shows routing depth da master (L0 = master, deeper levels further upstream). Each stage shows `% | ms` per track, and the table includes per-track enabled plugin count to spot heavy chains quickly.
...
These are per-track hints, not a global overload alarm - use the top DSP gauge for che.
