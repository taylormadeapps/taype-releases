...
**Stock** entries such as **Tape Rooms** and **Ache-Ritardo**.
...
Carica a plugin in one of a track's 8 insert slots. Requires transport
stopped.

| Param | Type | Required | Description |
|-------|------|----------|-------------|
| `track_id` | string | yes | Target track |
| `plugin_id` | string | yes | Plugin path or UID da `list_plugins`, or a bundled extension name/path |
...
Unload the plugin da a track's insert slot. Requires transport stopped.
...
List saved TayPE plug-in presets che match the current insert slot.
...
Carica a saved TayPE plug-in preset in the current insert slot. Requires
transport stopped.

| Param | Type | Required | Description |
|-------|------|----------|-------------|
| `track_id` | string | yes | Target track |
| `preset_name` | string | yes | Saved preset name/path da `list_insert_presets` |
| `slot` | number | no | Insert slot index 0-7 (default: 0) |

**Returns:** `{ "track_id": "...", "slot": 0, "preset_name": "Latenza Clean", "latency_samples": 4706 }`

### `open_insert_editor`

Apri the plugin editor window.
...
Chiudi the plugin editor window.
...
Abilita or disable tape summing. Requires transport stopped.
...
Apri the Softube Tape plugin editor window.
