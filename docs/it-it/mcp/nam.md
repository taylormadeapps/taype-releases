...
**Returns:** Array of results con id, name, creator, gear, `thumbnail_url`, download count, `architecture`, and `amx_eligible`. Architecture-filtered searches resolve those fields before filtering; unfiltered searches may return an empty `architecture` string when TONE3000 omits it.
