...
Carica a NAM profile on a track's preamp. Requires transport stopped.
...
List available NAM profiles da the local profiles directory.
...
```json
{
  ...
  "directory": "[TAPE_HOME]/NAM/Preamps",
  "count": 1
}
```
...
Carica a NAM summing profile on the master bus. Requires transport stopped.
...
Disabilita NAM summing, revert to digital sum. Requires transport stopped.
...
**Returns:** Array of results con id, name, creator, gear, `format`, `thumbnail_url`, download count, `architecture`, and `amx_eligible`. TONE3000 gear is the captured form factor; NAM/IR lives in `format`. Architecture-filtered searches resolve those fields before filtering; unfiltered searches may return an empty `architecture` string when TONE3000 omits it.

### `download_tone3000`

Download a NAM profile da TONE3000 to the local profiles directory.
Filenames are based on tone metadata (title/creator) for both `preamp`
and `summing`, con tone ID fallback when metadata is unavailable. If the
requested tone is already present locally con matching metadata, TayPE returns
che file instead of downloading a duplicate.

| Param | Type | Required | Description |
|-------|------|----------|-------------|
| `tone_id` | string | yes | TONE3000 tone ID (da search results) |
| `category` | string | no | "preamp" (default) or "summing" |
