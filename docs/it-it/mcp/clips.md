...
| `time` | number | no | Linea Temporale position in seconds (default: 0.0) |
| `duration` | number | no | Clip duration in seconds (default: full file length) |
| `clip_start` | number | no | Offset in source file in seconds |
...
| Param | Type | Required | Description |
|-------|------|----------|-------------|
| `track_id` | string | no | Filtro by track (omit for all) |
