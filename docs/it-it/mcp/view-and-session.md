...
Seleziona a track (or deselect all).

| Param | Type | Required | Description |
|-------|------|----------|-------------|
| `track_id` | string | no | Traccia ID (omit to deselect) |
...
Ripeti the last undone change. Same return shape as `undo`.
...
Set the active personality by name. See [Personalità](personalities.md).
...
all tracks con peak levels, call sign, undo/redo state, personality, theme,
view state, and metronome. Use questo to load context at the start of a conversation.
