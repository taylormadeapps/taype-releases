...
List all tracks con their channel settings.
...
|-------|------|----------|-------------|
| `name` | string | no | Traccia name (default: "Traccia N") |
...
| `preamp_manual_gain` | number | no | Manual output gain: Modern/NAM/MD510 -18.0 to +12.0 dB, ToTaype centred control -0.5 to +0.5 |
...
(buses receive da routed tracks, not the audio interface). The previous
input is stashed and restored when `is_bus` is later set to `false`, unless
an explicit `input_id` is provided in che same `set_track` call (explicit
input wins). Setting `is_bus: false` also clears any tracks whose
`output_id` or `sends` pointed at che track, preventing stale routes to
a non-bus.

`comp_bus: true` turns the track in a comp bus without changing the
underlying routing model. Child takes are still ordinary tracks routed to
che bus. `comp_input_id` stores the group's shared visible input choice,
and child take tracks inherit che in their real `input_id` while their
...
Compressore fields are also supported through `set_track`: `comp_enabled`,
`comp_threshold`, `comp_ratio`, `comp_attack_ms`, `comp_release_ms`,
`comp_knee`, `comp_makeup_gain`, `comp_wet_dry_mix`, `comp_log_release`, and
...
Rimuovi a track and all its clips.
