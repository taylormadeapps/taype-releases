...
コンプレッサー関連パラメータも `set_track` で扱えます: `comp_enabled`、`comp_threshold`、`comp_ratio`、`comp_attack_ms`、`comp_release_ms`、`comp_knee`、`comp_makeup_gain`、`comp_wet_dry_mix`、`comp_release_mode`、`comp_log_release`、`comp_sidechain_low_cut`（検出側サイドチェインの固定 `80 Hz` ハイパス）。`comp_attack_ms` は **0.1–100 ms** を受け付けます。
