...
## Kompresor Visualiser

The compressor header now includes the same small spectrum visualiser button used on the filter and Korektor headers, tucked immediately to the left of the section power glyph. Click it to open a floating compressor window for the current track.
...
The compressor popup now behaves like the Korektor popup: there is one shared floating window, and while it is open it follows the currently selected track. The header button stays lit on whichever track the shared window is currently targeting.

## Redukcja wzmocnienia Miernik
