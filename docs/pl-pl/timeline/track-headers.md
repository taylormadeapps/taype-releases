...
![Ścieżka headers](../../assets/img/docs/track-headers.png)

## Controls

- **Colour strip** - click to change the track colour from 24 swatches
- **Ścieżka name** - double-click to rename (only when stopped). With popup help enabled, hovering shows the full title.
...
- **Nowy Tag** opens inline creation with scope choice:
  - **Global** tags are shared across reels on this machine
  - **Reel** tags are saved in the reel and travel with it

The popup stays open while you assign multiple tags. Click outside or press **Gotowe** to close.
