...
Narzędzia for managing TayPE reel bundles, checkpoints, and archive packaging.
...
|-------|------|----------|-------------|
| `name` | string | no | Nowy reel name for duplicate-and-switch |

**Returns:** `{ "saved": true, "reel": "MyReel" }`

### `open_reel`

Otwórz an existing reel by name. TayPE persists the current reel's working state
before switching.
...
**Returns:** `{ "reels": ["Domyślne", "MyReel", "Demo"] }`
...
```json
{
  "loaded": true,
  "name": "Domyślne",
  "directory": "/Users/.../Reele/Domyślne.taype",
  "bundle_path": "/Users/.../Reele/Domyślne.taype",
  "reel_file": "/Users/.../Reele/Domyślne.taype/reel.xml",
  "sample_rate": 44100,
...
Rozładuj the current reel and leave TayPE in a no-reel state. TayPE flushes the
...
Duplikuj the current reel to a new `.taype` bundle and switch to it.

| Param | Type | Required | Description |
|-------|------|----------|-------------|
| `name` | string | yes | Nowy reel name |
...
Zmień nazwę the current reel bundle. The bundle filename is the canonical reel
name.
...
Utwórz an intentional checkpoint in the current reel history. TayPE flushes
working state first if needed.
...
| Param | Type | Required | Description |
|-------|------|----------|-------------|
| `id` | string | no | Punkt kontrolny ID |
| `name` | string | no | Punkt kontrolny display name |
...
Usuń a checkpoint tag and its index entry.
...
Utwórz a `.tpak` archive of the current reel.

| Param | Type | Required | Description |
|-------|------|----------|-------------|
| `destination` | string | no | Wyjście archive path; defaults beside the bundle |
...
Utwórz a new clean-slate `.taype` bundle from the current working state,
...
**Returns:** `{ "revealed": true, "path": "/Users/.../Reele/MyReel.taype" }`
