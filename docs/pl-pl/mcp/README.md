...
In TayPE, go to **Narzędzia > AI Integrations > Install Claude Connector**. This automatically
...
In TayPE, go to **Narzędzia > AI Integrations > Install Codex Connector**. This tells Codex to add
...
```json
{
  "mcpServers": {
    "taype": {
      "command": "/Applications/Taype.app/Zawartość/MacOS/taype-mcp"
    }
  }
}
```
...
- **Instructions** - a preamble with links to these docs, plus the active
  studio tech personality if one is set (see [Osobowości](personalities.md))
...
is stopped. Odtwarzanie is for listening and performing. This keeps the
engine predictable and prevents the kind of race conditions that crash
other DAWs.

Narzędzia that require stopped transport will return an error if you call them
...
release the lock from the Edycja menu. A successful `tx_commit` also persists
...
- [Cuty](cuts.md) - alternate timeline pages inside one reel
- [Reel Management](reels.md) - save, open, list reels
- [Ścieżki](tracks.md) - add, edit, remove tracks
- [Clipy](clips.md) - add, edit, remove clips
- [Nagrywanie](recording.md) - start and stop recording
- [Wtyczki](plugins.md) - VST3 inserts, sandbox, Mix FX
- [Widok & Session](view-and-session.md) - view state, undo, theme, personality, status
...
- [Osobowości](personalities.md) - studio tech characters that shape the AI interaction
