...
Narzędzia for view state, undo/redo, theme, personality, and session status.
...
Get the current visibility and display state. Fokus, Archiwum Widok, Rozlanie,
...
Toggle Fokus mode (shows only relevant tracks).

| Param | Type | Required | Description |
|-------|------|----------|-------------|
| `enabled` | boolean | yes | Fokus on/off |
| `tag` | string | no | Optional focus tag (Fokus-by-Tag) |
| `tags` | string[] | no | Optional multiple focus tags |
| `filter_mode` | string | no | `"or"` (default) or `"and"` for multi-tag focus |

### `set_archive_view`

Toggle Archiwum Widok (show archived tracks).

| Param | Type | Required | Description |
|-------|------|----------|-------------|
| `enabled` | boolean | yes | Archiwum view on/off |

### `set_spill`

Toggle Rozlanie (show tracks routed to the selected bus or master).

| Param | Type | Required | Description |
|-------|------|----------|-------------|
| `enabled` | boolean | yes | Rozlanie on/off |

Requires a bus or master track to be selected.

Fokus, Archiwum Widok, Rozlanie, and tag-filter state are not reel data and reset
on relaunch. Ruler mode, automation view, and meter scale still persist as
app-global preferences.

### `set_e_zoom`

Toggle E-Powiększenie (zoom to selected clip).

| Param | Type | Required | Description |
|-------|------|----------|-------------|
| `enabled` | boolean | yes | E-Powiększenie on/off |
...
Wybierz a track (or deselect all).

| Param | Type | Required | Description |
|-------|------|----------|-------------|
| `track_id` | string | no | Ścieżka ID (omit to deselect) |
...
```json
{
  ...
  "redoLabel": "Dodaj track"
}
```

### `redo`

Powtórz the last undone change. Same return shape as `undo`.
...
Set the active personality by name. See [Osobowości](personalities.md).

| Param | Type | Required | Description |
|-------|------|----------|-------------|
| `name` | string | yes | Osobowość name |
...
Comprehensive session snapshot - reel info, transport, selected Wytnij, Wytnij list,
...
| Param | Type | Required | Description |
|-------|------|----------|-------------|
| `call_sign` | string | yes (set only) | Nowy call sign |
