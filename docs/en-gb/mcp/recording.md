...
stop mode. `get_loop_record_mode` / `set_loop_record_mode` control whether
loop braces auto-punch, wrap during record, or run Sooper-Looper.
`record_start` / `record_stop` stay low-level and let MCP clients drive the
take lifecycle directly.

`sooper_looper` is a loop-record mode. The UI record button snapshots the
armed tracks in visible order and walks them one at a time across the active
loop; low-level `record_start` still records the supplied target list
directly.
...
| Param | Type | Required | Description |
|-------|------|----------|-------------|
| `loop_record_mode` | string | yes | `"auto_punch"`, `"loop"`, or `"sooper_looper"` |

**Returns:**
```json
{
  "loop_record_mode": "sooper_looper"
}
```
