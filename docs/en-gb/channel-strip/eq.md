...
The graph body's own help now spells out the handle gestures too: drag
horizontally to sweep frequency or cutoff, drag vertically on a bell to boost
or cut, and **Option-dragging** a bell handle in that popup solos the swept
band while you move it. Releasing keeps the newly swept centre frequency
instead of dropping the handle back on stale pre-solo coordinates. Hold
**Shift** while dragging any popup knob for a finer 4x trim pass.
